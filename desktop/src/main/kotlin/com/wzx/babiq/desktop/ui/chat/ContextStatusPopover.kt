...
fun ContextStatusPopover(
	...
	Card(
		...
		Column(
			...
			Text("本轮上下文窗口", style = MaterialTheme.typography.titleMedium.copy(fontWeight = FontWeight.Bold))
			Text(
				"当前用户问题始终最高优先级，历史、记忆和能力目录只作为参考层注入。",
				style = MaterialTheme.typography.bodySmall,
				color = BaBiQColors.Muted,
			)
			Row(horizontalArrangement = Arrangement.spacedBy(8.dp), modifier = Modifier.fillMaxWidth()) {
				MetricTile("模型窗口", status?.modelContextWindow?.let(::formatInteger) ?: "未加载", Modifier.weight(1f))
				MetricTile("当前估算", status?.lastEstimatedTokens?.let(::formatInteger) ?: "未生成", Modifier.weight(1f))
				MetricTile("自动压缩阈值", status?.autoCompactThreshold?.let { threshold ->
					if ((status.modelContextWindow) > 0) percentageLabel(threshold.toDouble() / status.modelContextWindow) else formatInteger(threshold)
				} ?: "未配置", Modifier.weight(1f))
			}
			HorizontalDivider(color = BaBiQColors.Border)
			buildContextStatusPopoverEntries(context, memory, capability).forEach { entry ->
			...
			Text(
				"旧历史会以 REPLACED_BY_SUMMARY 等原因保留审计记录；Skill 正文只在显式读取或命中后进入上下文。",
