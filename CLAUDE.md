...
- P3-4 长期记忆异步流水线已完成：
  ...
  - `docs/superpowers/plans/p3-4-long-term-memory/codex-handoff.md`
  - 已按 Context7 核对 Spring AI structured output、ChatMemory/MessageWindowChatMemory 边界和 Spring AI Alibaba Agent Framework/Hook/Interceptor/MemorySaver/上下文治理能力；已核对 Codex 源码两阶段长期记忆流水线、启动 idle 扫描、70% 输入预算和 read path summary-only 注入。
  - 后端新增 V10 migration、`bq_memory_jobs`、`bq_memory_candidates`、`bq_memory_artifacts`、`bq_memory_references`，并同步 SQL 中文注释、`bq_schema_comments`、Entity 注释和覆盖测试。
...
- P3-5 按需能力装配、记忆检索增强和桌面控制已完成：
  ...
  - 本阶段保持 Spring AI `1.1.6`、Spring AI Alibaba `1.1.2.3` 不升级；Dynamic Tool Search 采用 BaBiQ 自有 `CapabilitySearchService` 端口和 `tool_search` 工具实现。
...
- P3-5a Lucene 能力搜索替换已完成：
  - `docs/superpowers/plans/p3-5a-lucene-capability-search/plan.md`
  - `docs/superpowers/plans/p3-5a-lucene-capability-search/codex-handoff.md`
  - 已按 Context7 核对 Spring AI Dynamic Tool Search、Spring AI Alibaba ReactAgent/Hook 边界和 Apache Lucene StandardAnalyzer/BM25 行为；实际依赖树也已核对 `org.springaicommunity:tool-searcher-lucene:1.0.1`。
...
- P4 Plan/Todo 可视化已完成：
  ...
  - 已按 Context7 核对 Spring AI `@Tool` / `ToolContext` 和 Spring AI Alibaba ReactAgent/system prompt 接入边界；已核对 Codex `update_plan` 与 Claude Code `TodoWrite` 源码语义。
  - 后端新增 `update_plan` 本地工具，扩展 `PlanItem` 步骤状态与 `activeForm`，首发 `item/added`、续发 `item/updated`，不新增数据库表。
  - `update_plan` 默认 VISIBLE，补齐中文能力别名；不在写类工具集合，不触发审批；简单任务是否出现计划只由 system prompt 约束，不写复杂度检测代码。
  - 桌面端新增 `ThreadItem.Plan`、`PlanUiState` 和右侧运行面板进度区；plan item 不进入聊天消息流，全部完成后自动隐藏。
  - 已验证：`cd backend; .\mvnw.cmd "-Dtest=UpdatePlanToolTest,ThreadItemJsonTest,CapabilityAliasDictionaryTest,CapabilityCatalogSyncServiceTest,ReActStrategyTest,SystemPromptSecurityRuleTest,AgentLoopLineCountTest" test`。
  - 已验证：`cd backend; .\mvnw.cmd clean verify`。
...
- P6-1 只读子 Agent 委派已全量闭环（代码 + 自动化验收 + 真实模型人工烟测，2026-06-01）：
  ...
  - 行为验收已补齐并通过：`SubAgentDelegationTest`（委派回传 + 子事件折叠不外泄）、`SubAgentModelResolutionTest`（模型 inherit/override）、`SubAgentDelegationIT`（端到端只读 + 沙箱拦 write_file + 运行记录归属）；`cd backend; .\mvnw.cmd clean verify` 全绿（含 23 IT）。
  - 真实模型人工烟测已通过（2026-06-01）：主 Agent 会主动委派 explorer 子 Agent 并执行。
- P6 规划链已齐备（master + 4 子阶段，均经 Context7 + 本地 jar 核对）：
  ...
  - P6-2 flow 编排计划就绪（`p6-2-flow-orchestration/`）：薄封装 `SequentialAgent / ParallelAgent / LlmRoutingAgent`，节点可读可写并行真实工作；写用 approve-once（运行前整体批准 + 沙箱，4 条安全语义：沙箱硬边界 / 弹窗列清范围 / 批准后 flow 冻结 / 危险不可逆操作由沙箱禁止）；运行中逐工具审批 + 并发中断留 P6-2b。
...
- **下一步**：实现 **P6-2 flow 编排**（plan + codex-handoff 已就绪，前置 P6-1 已完成）；可选先做 P6-1b 写类委派（`asNode` + 审批上浮）。P6-3 实现前须先按 plan 用 graph-core 原语搭出 supervisor 图。
...
- P2-1、P2-2、P2-3、P2-4、P2-5、P2-6 已完成；用户已暂时验收 P2，P3-1 最小上下文底座、P3-2 当前窗口管理运行时、P3-3 短期记忆/上下文压缩、P3-3a 鲁棒性补强、P3-4 长期记忆异步流水线、P3-5 按需能力装配、P3-5a Lucene 能力搜索替换、P4 Plan/Todo 可视化、P5 ReasoningItem 接通、P6-1 只读子 Agent 委派已全量闭环（代码 + 自动化 + 真实烟测）；P6-0 spike 完成、P6-2/P6-3 计划就绪。
