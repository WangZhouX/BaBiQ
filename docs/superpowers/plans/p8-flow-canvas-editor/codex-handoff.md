...
## 2026-06-14 追加修复：画布基础交互可用性

- 发现根因：旧画布把“插到组内”和“插到组后”都降级成 `anchorNodeId`，当锚点位于并行/路由组内时，串行新增和拖拽移动会误改组结构，表现为后续节点全部变成并行/路由分支。
- 修复规则：新增 `FlowInsertTarget`，显式区分 `Append`、`AfterNode`、`AfterGroup`、`IntoGroup`；布局层每个线上“+”携带真实目标，UI 不再用最后一个 nodeId 猜测插入位置。
- 交互语义：组内分叉线的“+”只负责给当前并行/路由组加分支；组后串行线的“+”负责在父级顺序流里加下一步；拖拽到组内节点之后默认移动到该组之后，不再把串行节点塞回并行/路由组。
- 已补回归：`FlowGraphModelTest` 覆盖组后串行、显式组内并行、组后并行不污染原组、拖拽到组内节点后仍位于父级顺序；`FlowCanvasLayoutTest` 覆盖分支插入目标与下一步插入目标分离。
- 追加修复：START 到第一个节点之间的“+”改为 `Prepend`，串行新增会插到第一个节点前；创建菜单和默认新增动作只暴露串行/并行，路由仅保留为历史结构和后端兼容能力，不作为当前 P8 用户入口。
- 追加修复：画布空白区域拖拽改为显式相机平移，按下时先命中测试节点/终端/插入按钮，只有真正空白才消费拖拽；节点拖拽仍然用于重排节点。新增 `flowCanvasScreenToLayoutPoint` 与 `flowCanvasHitTarget` 回归，避免节点多时只能靠缩放查看。
