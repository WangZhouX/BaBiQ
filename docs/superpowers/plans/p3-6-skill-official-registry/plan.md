...
- **Step 1b — 路径与配置**：`SkillProperties` 默认改为 `~/.agents/skills`（用户级）+ `<cwd>/.agents/skills`（项目级），新增可选 `additional-directories`（**默认空**），去掉 `~/.codex` 写死默认（**breaking，D9**）。补 `SkillPropertiesTest` 断言"默认仅 `.agents/skills`、`additional-directories` 默认空""把 `~/.codex/skills` 加进配置后旧技能恢复加载"。
...
- 真实烟测（可选）：放含 front-matter 的 `SKILL.md` 到 `~/.agents/skills` 与 `<cwd>/.agents/skills`，`skills/list` 列出、`skills/get` 读正文且按 16000 字符截断、`tool_search` 中文 query 命中、**改写 `SKILL.md` 后重新 `list` 立即生效**、把 `~/.codex/skills` 加进 `additional-directories` 后旧技能恢复加载（breaking 迁移验证）。
...
- ✅ 实现：**交 Codex**，按 `codex-handoff.md`（含 §11 审查修订段）TDD 落地。
