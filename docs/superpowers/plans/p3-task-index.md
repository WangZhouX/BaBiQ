...
| P3-2 | 待计划 | 当前窗口管理运行时、持久化快照和 Agent 前置接入 | 后续创建 | P3-1 |
...
- P3-2 先做可审计 current window、持久化 context snapshot 和 Agent 前置接入，不急着压缩。
- P3-3 再接 short-term compaction，必须能恢复、能审计、能在 UI 看到 `ContextCompactionItem`。
