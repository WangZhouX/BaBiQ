...
>
> 当前结论：登录页、OA 认证底座、READY 门禁、前后端独立运行配置及 `code=0/200` 兼容已经提交；最新分离烟测发现“新连接首次发送未登录身份通知”被后端拒绝，修复代码和两组回归测试目前仍未提交。Goal 必须保持 `active`。

## 1. 下一位 Agent 的任务

不要重做已经完成的登录迁移。接手当前 3 个未提交文件，审查并完成“首次 signed-out 身份通知”修复；随后重新执行后端与桌面端全量测试、前后端分离烟测、敏感信息扫描和独立代码审查，更新最终验收记录并使用中文 commit。没有真实 OA 账号时，不得声称正确密码登录、Token 刷新、重启恢复、主动退出和登录后 Agent 的真实链路已经人工验收通过。
...
```text
M backend/src/main/java/com/wzx/babiq/server/application/auth/ApplicationIdentityRegistry.java
M backend/src/test/java/com/wzx/babiq/server/application/api/ApplicationIdentityCatalogHandlersTest.java
M backend/src/test/java/com/wzx/babiq/server/application/auth/ApplicationIdentityRegistryTest.java
```

- 这 3 个文件是同一项 TDD 修复，不是无关用户改动；不要丢弃、reset 或 checkout。
- 本交接文档应单独提交，不要顺手提交上述 3 个功能文件。
...
以下全量结果包含 `fda372a`，但不包含当前未提交的后端 signed-out 修复。
...
注意：当前 3 个未提交后端文件修改后，必须重新运行后端全量；桌面端测试会打包 backend jar，最终也应重新运行桌面端全量。
...
### 7.4 下一位 Agent 必须先做的事

1. 阅读 3 个未提交文件的完整 diff。
2. 重点审查 `acceptInitialSignedOut` 的并发和锁语义：
   - 生产 bind handler 已在 `synchronized(connection)` 中执行。
   - update 也在 connection 锁内执行。
   - 当前 cleanup 后二次检查用于拒绝异常并发写入。
3. 运行更宽的身份定向测试，例如：
...
4. 确认没有回归后，用中文提交这 3 个文件，建议：

```text
fix(登录): 接受首次未登录身份通知
```

不要在审查和测试前直接提交，也不要把本交接文档混进该功能提交。

## 8. 尚未完成或不能声称完成的验收

仍需完成：

- 当前 signed-out 修复的更宽定向测试。
- 当前 signed-out 修复后的后端 `clean verify`。
- 当前 signed-out 修复后的桌面端 `clean test`。
...
2. 完整读取第 3 节材料，核对 `git status --short` 和 3 个未提交文件。
3. 确认没有残留 Gradle/Java 前后端进程正在写 `business-desktop/**/build`。
4. 按第 7.4 节审查并验证首次 signed-out 修复。
5. 用中文 commit 单独提交 3 个功能/测试文件。
6. 运行后端全量：
...
8. 新建全新的隔离 runtime，严格从 `.run/Business Backend.run.xml` 与 `.run/Business Frontend.run.xml` 解析配置，重新做前后端分离烟测。
9. 在新烟测中确认：
...
14. 执行：
    ...
    - `git status --short`
    - 最终敏感键与金丝雀扫描。
15. 使用中文 commit 收口文档，建议：

```text
test(登录): 完成 OA 登录验收记录
```

16. 只有上述自动化、分离烟测、安全扫描、独立审查和文档全部完成后，才能：
...
不要重做已完成的 OA 登录功能。当前 fda372a 已用 TDD 修复并提交 code=0/200 兼容。工作区另有 3 个未提交文件，正在修复“新连接首次 application/identity/update(signed-out) 被后端拒绝”；定向测试已经经历 24 tests / 2 errors 的 RED，并达到 24/24 GREEN。先审查这 3 个文件的并发与身份水位语义，运行更宽定向测试，再用中文 commit 单独提交。之后必须重新跑 backend clean verify、business-desktop clean test、全新 runtime 的前后端分离烟测、身份错误日志断言、敏感信息扫描和独立代码审查，最后更新设计/计划/验收文档。
