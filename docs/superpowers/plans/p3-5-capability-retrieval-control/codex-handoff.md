...
P3-5 已进入实现收口。本阶段保持 Spring AI `1.1.6` 和 Spring AI Alibaba `1.1.2.3` 不升级，采用 BaBiQ 自有 `CapabilitySearchService` / `tool_search` / SQLite 审计链路完成按需能力装配，并把长期记忆检索增强接入 `long_term_memory` 参考层。
