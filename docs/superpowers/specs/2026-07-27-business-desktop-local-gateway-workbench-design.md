...
OA session 绑定 desktop instance/session，而不是某一个瞬时 WebSocket 对象。WebSocket 断开后，
durable session 进入 `DETACHED`，旧 live lease 失效，但 credential 和非敏感索引仍可恢复。
...
- `business/auth/session/attach` 只附着当前 desktop instance/session 已拥有的 `DETACHED` durable session，
  进入 `RESTORING -> INSTALLING`，重新校验权限并由服务端原子安装 READY；不能跨 desktop session 抢占。
...
child process，不读取或解析远程地址。IDEA 独立启动后端时使用同一文件作为
`spring.config.additional-location`，不依赖工作目录或 C 盘复制。
...
OA 后端 `LawNoticePageReqVO` 继承 `PageParam`，真实字段是 `pageNo/pageSize`。现有 Web 发送
`page=1` 时未知字段被 Spring 忽略，只因 `pageNo` 默认值为 1 才碰巧取得第一页；桌面 BFF 必须使用
真实 `pageNo=1` 合同，不能复制这个前端偏差。
...
远程图片 URL 只从可信 OA 响应注册成随机 opaque handle；客户端不能提交任意 URL。后端执行协议、
DNS/private-address、重定向、内容类型和大小校验，按 session/generation 做短期缓存。
...
并移除文件名路径。通过后流式转发到 `/infra/file/upload-return-ids`，固定服务端注入
`fileStorageName=ht-law-file-management`。服务端状态机固定为：

```text
PREPARED -> UPLOADING -> READY -> CONSUMED
                |          |
                +-> FAILED +-> REVOKED
                +-> OUTCOME_UNKNOWN
```

- 领取上传使用 CAS，只有一个请求能从 `PREPARED` 进入 `UPLOADING`；ticket 不可重复消费。
- 所有声明文件均完成实际字节数、MIME/扩展名和可选 hash 校验，且 OA 返回完整 fileId 集合后才进入
  `READY`；部分上传永不成为 READY。
- 远端已发送但响应不确定时进入 `OUTCOME_UNKNOWN`，不自动重放，也不能被 schedule/create 消费。
- `business/schedule/create` 只接受同一 operation、父资源授权、desktop/OA session 和 generation 的
  `attachmentBatchId`，CAS 单次转换到 `CONSUMED`；Compose 永远看不到 OA fileId。
- 失败、退出、过期、父授权变化或 generation 变化转为 `FAILED/REVOKED`，并清理受控临时文件。

HTTP 日志只记录 correlationId、状态和计数，不记录 ticket、文件名、路径、SHA、multipart 内容或 OA
响应正文；HTTP 错误同样使用固定安全映射，不能绕过 JSON-RPC 的脱敏规则。
...
权限接口的 menu tree 在后端转换为稳定 DTO，仅允许当前 Web 左栏 allowlist 的可见一级路径，首页
始终保留。其他菜单和以下入口只展示占位：
