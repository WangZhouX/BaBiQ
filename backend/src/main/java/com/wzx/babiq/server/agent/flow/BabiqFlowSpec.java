 ...
 * 写入范围和沙箱模式都被固定下来。后续执行只读取这份规格，不允许运行中增删节点，
 * 从而避免“审批的是 A，执行的是 B”的权限漂移。</p>
 *
 * @param orchestrationId 流程 id，贯穿协议 item、运行记录和持久化表
 * @param title 用户可读标题
 * @param topology 顺序、并行或路由
 * @param nodes 节点列表，构造时按 order 排序并去重校验
 * @param mergeOutputKey Spring AI Alibaba parallel merge 输出键
...
public record BabiqFlowSpec(
    ...
    /**
     * 规范化流程规格，确保后续装配官方 FlowAgent 时拿到稳定顺序和不可变列表。
     */
    public BabiqFlowSpec {
        ...
        if ((topology == BabiqFlowTopology.PARALLEL || topology == BabiqFlowTopology.ROUTING) && nodes.size() < 2) {
            throw new IllegalArgumentException(topology + " 流程至少两个节点");
        }
        assertUnique(nodes);
    ...
    public BabiqFlowSpec withNodes(List<BabiqFlowNode> replacement) {
        ...
        return new BabiqFlowSpec(orchestrationId, title, topology, replacement, mergeOutputKey, approved, false, sandboxMode);
    }
    ...
    public BabiqFlowSpec approvedAndFrozen(SandboxMode effectiveSandboxMode) {
        return new BabiqFlowSpec(orchestrationId, title, topology, nodes, mergeOutputKey, true, true, effectiveSandboxMode);
    }
