...
public class ContextWindowRuntime {
    ...
    private static final double AUTO_COMPACT_RATIO = 0.70d;
    ...
    public ContextWindowRuntimeResult prepare(ContextWindowRuntimeInput input) {
        ...
        int threshold = (int) Math.floor(modelWindow * AUTO_COMPACT_RATIO);
        CapabilityCatalog capabilityCatalog = capabilityCatalogAssembler.assemble(input.toolCallbacks());
        ContextAssemblyResult assemblyResult = contextAssembler.assemble(new ContextAssemblyInput(
                input.threadId(),
                input.turnId(),
                input.userText(),
                input.cwd(),
        ...
        ContextSnapshotRecord snapshot = snapshotRecord(input, assemblyResult, capabilityCatalog,
                snapshotId, existingWindow, modelWindow, threshold, now);
        String modelInputText = promptRenderer.render(assemblyResult);
        try {
            snapshotRepository.save(snapshot);
            windowRepository.upsert(windowRecord(input, existingWindow, snapshotId, modelWindow, threshold, now));
    ...
    private ContextSnapshotRecord snapshotRecord(ContextWindowRuntimeInput input,
        ...
        int ordinal = existingWindow == null ? 0 : existingWindow.windowOrdinal();
    ...
    private ContextWindowRecord windowRecord(ContextWindowRuntimeInput input,
        ...
        return new ContextWindowRecord(
                input.threadId(),
                existingWindow == null ? 0 : existingWindow.windowOrdinal(),
                existingWindow == null ? null : existingWindow.activeSummaryId(),
                modelWindow,
                threshold,
                snapshotId,
